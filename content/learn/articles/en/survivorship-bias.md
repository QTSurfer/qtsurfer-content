...
- When a pair's history is not managed by the platform, upload it as a dataset: a CSV, parquet, or
  lastra file with a `timestamp` and `close` column, optionally with open, high, low, and volume, is
