...
[prepare/execute](backtest_execute) exactly as normal but with the reserved `exchangeId: user`.
...
count limit is reached, or your account's total storage limit ([`GET /account`](account)'s
...
[`docs/backtest_execute.md`](backtest_execute) for the full prepare/execute reference,
